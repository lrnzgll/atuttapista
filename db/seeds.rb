...
csv.each do |row|
  ...
  if county
    ...
    town.update(latitude: 45, longitude: 5)
    p town
  end
end
p 'towns created'
...
Array.new(100) do |i|
  r = Route.create(name: "#{Faker::Address.city} to #{Faker::Address.city}", description: BetterLorem.p(rand(3..20), true, true ), difficulty: rand(1..10), distance: rand(3..100), gpx: 'bla', user: User.all.sample)
